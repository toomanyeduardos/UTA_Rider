...
- (NSData *)getVehicleMonitorForRouteNumber:(NSString *)routeNumber
{
    ...
    urlAsString = [urlAsString stringByAppendingString:@"UMM4NBIBJUJ"];         // My token
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    if ([currentElement isEqualToString:@"DataFrameRef"])
    {
        [dataFrameRef_arr addObject:string];
    }
    if ([currentElement isEqualToString:@"DatedVehicleJourneyRef"])
    {
        [datedVehicleJourneyRef_arr addObject:string];
    }
    if ([currentElement isEqualToString:@"PublishedLineName"])
    {
        [publishedLineName_arr addObject:string];
    }
    if ([currentElement isEqualToString:@"OriginRef"])
    {
        [originRef_arr addObject:string];
    }
    if ([currentElement isEqualToString:@"DestinationRef"])
    {
